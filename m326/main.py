...
etree = html.etree
baseurl_search = r'http://www.m326.com/so.php?wd='
baseurl_next_page = r'http://www.m326.com/so.php?wd={*}&page={#}'
baseurl_homepage = r'http://www.m326.com'
headers = {
...
def get_search_result(keyword):
    ...
    while True:
        page += 1
        url_next_page = baseurl_search + f'{keyword}&page={page}'  # 替换字符
        response = requests.get(url_next_page, headers=headers)
        if response.status_code == 200:
            html_str = response.text
            urls_page = get_urls(html_str)
            if urls_page:
                urls_join += urls_page
            else:
                break
...
def get_urls(html_str: str):
    """利用正则提取网页链接"""
    pattern = r'<a href="(/sing/.+?html)" target'
    short_urls = re.findall(pattern, html_str)  # 短链接/sing/a8446e5f87f8df2c92942b88385bea25.html
...
def main():
    while True:
        ...
        urls = get_search_result(keyword)
